...
@register("fields_truthy")
def fields_truthy(ctx, p):
    """All records in the optional `where` scope have truthy values for every listed field."""
    ...
    findings = []
    for r in scope:
        missing = [f for f in p["fields"] if not bool(r.get(f))]
        if missing:
            findings.append({**r, "_reason": f"false or missing {missing}"})
    if findings:
        return CheckResult("FAIL", f"{len(findings)} of {n} in-scope records have false or missing required fields",
                           findings, examined=N, in_scope=n)
    return CheckResult("PASS", f"{n} of {N} records have all required fields truthy",
