...
Three rules the code enforces, each of which exists because the obvious alternative is wrong:
  ...
  4. (WB-102) Mutual `not_applicable` is scope agreement, not requirement agreement. Both sides
     saying an element does not apply says nothing about whether any requirement was met, so it
     is reported as `n_scope_agreed` and excluded from the compared population. It previously
     counted as an agreement, which inflated the rate on any contract carrying conditional
     elements — M3.6 now has five of fourteen written as "where relevant".
"""
...
def compare(blind: dict, ai: dict | None, control, min_compared: int = MIN_COMPARED) -> dict:
    ...
    rows, disagreements = [], []
    n_agree = n_compared = r_unset = a_unset = n_scope_agreed = 0
    for e in elements:
        r = rv.get(e["id"], "unset")
        a = av.get(e["id"], "unset")
        r_unset += r == "unset"
        a_unset += a == "unset"
        scope_agreed = r == "not_applicable" and a == "not_applicable"
        compared = r != "unset" and a != "unset" and not scope_agreed
        agree = compared and r == a
        n_scope_agreed += scope_agreed
        if compared:
            n_compared += 1
            n_agree += agree
            if not agree:
        ...
        rows.append({
            ...
            "direction": ("scope_agreed" if scope_agreed
                          else _direction(r, a) if compared else "not_compared"),
    ...
    return {
        ...
        "summary": {
            ...
            "n_agree": n_agree,
            # UI/report compatibility alias: the semantic field is n_agree, but older
            # projections read "agreements". Keep one deterministic value rather than
            # letting the dashboard silently default to zero.
            "agreements": n_agree,
            "n_disagree": len(disagreements),
            "reviewer_unset": r_unset,
            "assessor_unset": a_unset,
            "n_scope_agreed": n_scope_agreed,
