...
TEMPLATE_MARKERS = re.compile(r"\b(REPLACE|TBD|TODO|FIXME|XXX|PLACEHOLDER|LOREM)\b|<[A-Za-z][A-Za-z_]{2,}>")
_TRAIL_PUNCT = re.compile(r"[.\s,!?:;]+$")

INDEPENDENT_SIGNALS = (
    "evidence", "record", "records", "log", "logs", "ticket", "report", "screenshot",
    "policy", "procedure", "missing", "not provided", "does not show", "insufficient",
    "contradict", "requirement", "element", "m3.", "e14", "e0", "c0",
)
...
def reason_error(reason: str, *, rating: str | None = None, revised: bool = False,
    ...
    if any(re.search(r"(?<!\w)" + re.escape(t) + r"(?!\w)", low) for t in DEFERRAL) and len(r) < DEFERRAL_CEILING:
        if not any(signal in low for signal in INDEPENDENT_SIGNALS):
    ...
    if revised and previous and low == _canon_text(previous):
