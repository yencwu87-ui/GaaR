...
def _where(e: dict, case: str = "", doc: dict | None = None) -> str:
    """Why this case got this verdict — per case where the record says so.

    GE-110b.1: this used to read only the element-level `where`, so the same sentence was
    rendered into every case row. Case `a`'s `e4 = Y` and case `b`'s `e4 = N` carried identical
    text, which is what `corpus_adequacy` flags as `elements_with_shared_justification`.

    A per-case judgement in `judgements:` wins. Falling back to the element note is marked
    `↩ element-level` in the cell rather than presented as though it were about this case —
    the whole point of the repair is that the gap stays visible until a person closes it.
    """
    if case and doc:
        from eval.label_provenance import justification_for
        text, prov = justification_for(doc, case, str(e.get("id") or ""))
        if text:
            t = " ".join(text.split())
            t = t if len(t) <= WHERE_MAX else t[:WHERE_MAX].rstrip() + " …"
            return t if prov == "per_case" else f"↩ element-level — {t}"
    ...
    for i, k in enumerate(DOCS):
        ...
        for e in els:
            ...
            if ov_by_id and eid not in scored_ids:
                continue
            v = str(e.get(k, ""))
            out.append(f"| `{eid}` | {v} | {_where(e, k, spec)} |" if show_where
                       else f"| `{eid}` | {v} |")
        out.append("")
        if unconfirmed:
