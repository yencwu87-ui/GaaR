...
from governance.evidence_intelligence import SufficiencyGate
...
def match_controls(index: Index, controls: list, *, min_ratio: float = MIN_RATIO,
    ...
    """
    out = {}
    enforce_gate = __import__("os").environ.get("WB_EVIDENCE_GATE_ENFORCE", "0").strip().lower() in {"1", "true", "yes", "on"}
    from governance.evidence_intelligence import SufficiencyGate
    gate = SufficiencyGate()
    for c in controls:
        hits = index.query(c, k=k, min_ratio=min_ratio, min_score=min_score,
                           min_bi_cosine=(float(__import__("os").environ["WB_EVIDENCE_MIN_BI_COSINE"])
                                          if __import__("os").environ.get("WB_EVIDENCE_MIN_BI_COSINE") else None))
        if hits and (not enforce_gate or gate.evaluate(hits).status == "PASS"):
            out[c.key] = hits
    return out


def build_evidence(control, hits, signals) -> dict:
    """Assemble evidence plus an auditable retrieval/sufficiency receipt."""
    rows = list(hits or [])
    text = "\n\n".join(f"--- Source: {ch.label} ---\n{ch.text}" for ch, _ in rows)
    sg = signals_for(control, signals)
    if sg:
        text += "\n\n--- Environment signals ---\n" + "\n".join(f"{x.path}: {x.hint}" for x in sg)
    gate = SufficiencyGate().evaluate(rows)
    retrieval_rows = []
    for row in rows:
        retrieval_rows.append({
            "path": str(row.chunk.path),
            "chunk_id": f"{row.chunk.path}#{row.chunk.idx}",
            "bm25_score": float(row.bm25_score),
            "bi_cosine": row.bi_cosine,
            "cross_score": row.cross_score,
            "methods": list(row.retrieval_methods),
    ...
    return {
        ...
        "file_name": "",
        "auto": True,
        "sources": [ch.path for ch, _ in rows],
        "retrieval": {
            "schema": "evidence-intelligence.1",
            "matches": retrieval_rows,
            "sufficiency": gate.to_dict(),
        },
    }
...
def propose(control, evidence: dict, pdf_bytes: bytes | None = None,
