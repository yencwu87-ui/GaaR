...
# what state is the workbench in right now, and what is the one next thing to do. Every number
# on it is read from state the guide already observes — nothing here calls the assessor, and
# nothing here writes. Streamlit cannot switch tabs programmatically, so the page names the tab
# to open rather than pretending a card is a link.
import html as _html

# Grouped by what the reviewer is trying to do, not by build order.
_HOME_LANES = [
    ("Assess", "#FFB347", [
        ("Scan", "Index an evidence folder, match it to controls and queue hidden proposals."),
        ("Review", "Record your own reading first, then see the AI proposal and decide."),
    ]),
    ("Automate", "#7C3AED", [
        ("AI Auditor", "Runs machine-owned audit skills to the next human checkpoint."),
        ("Autopilot", "Trigger and job ledgers. Advances machine-owned nodes only."),
        ("Watcher", "Discovers regulatory publications for you to confirm once."),
    ]),
    ("Govern", "#0EA5E9", [
        ("Living Results", "Current state projected from sealed GovernanceResults."),
        ("Outcomes", "Outcome posture computed from governed mappings."),
        ("Change", "Regulatory change review against the control library."),
        ("Engine", "Projection of the governed engine and its plan."),
        ("Operations", "Operational view across cycles and jobs."),
    ]),
    ("Prove", "#16A34A", [
        ("Audit", "Retrieval trace for the assessor and challenger."),
        ("Report", "Readiness summary for the organisation in scope."),
        ("Lifecycle", "Playbook plays, design rating against operation."),
        ("Measurement", "Measurement runs over the assessor."),
        ("History", "Every event in the append-only ledger, per control."),
    ]),
]


def _home_next_step(ctx: dict) -> tuple[str, str, str]:
    """(title, detail, tab). Ordered: the first unmet precondition wins."""
    if not ctx["controls"]:
        return ("Load the control library",
                "Upload the AI Governance Playbook workbook under Administration in the sidebar, "
                "or place it in the data/ folder.", "Sidebar")
    if not (ctx["reviewer"] or "").strip():
        return ("Name the reviewer",
                "Decisions carry the reviewer's name. Enter it in the sidebar before recording one.",
                "Sidebar")
    if ctx["ai_proposals"] == 0 and ctx["decisions"] == 0:
        return ("Bring in evidence",
                "Index an evidence folder or load the synthetic pack, then assess the matched controls.",
                "Scan")
    if ctx["ai_proposals"] > ctx["decisions"]:
        n = ctx["ai_proposals"] - ctx["decisions"]
        return (f"Review {n} waiting proposal{'s' if n != 1 else ''}",
                "Each proposal stays hidden until you have recorded your own reading of the evidence.",
                "Review")
    if ctx["open_failures"] or ctx["open_inconclusive"]:
        return ("Work the open audit results",
                f"{ctx['open_failures']} failing and {ctx['open_inconclusive']} not-testable results "
                "in the latest evidence bundle.", "Audit")
    return ("Produce the readiness summary",
