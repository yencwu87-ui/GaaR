...
def create_dossier(*, control_id: str, control_title: str, reviewer: str, blind: dict,
                   ...
                   assessment_id: str | None = None, reasoning_schema: str | None = None,
                   falsification_engine: dict | None = None,
                   validation_status: str = "") -> dict:
    """Create a dossier snapshot. Challenge responses start as open and are filled by the reviewer.

    WB-107: `validation_status` decides whether an empty challenge list means "the challenger had
    nothing to say" or "the challenger said things and none of them survived validation". The
    dossier reported both as `no_challenges`, which is the reading a later auditor would take at
    face value — an unchallenged reading that was in fact never challenged.
    """
    if not reviewer.strip():
        raise ValueError("a named reviewer is required")
    if not isinstance(challenges, list):
        raise ValueError("challenges must be a list")
    _blocked = str(validation_status or "").lower() == "blocked"
    dossier = {
        ...
        "falsification_engine": dict(falsification_engine or {}),
        "challenge_outcome": ("substantive" if any((c.get("challenge_strength") == "strong") for c in challenges)
                              else "weak_or_refining" if challenges
                              else "blocked" if _blocked else "no_challenges"),
        "challenges": [],
        "status": "open" if challenges else ("blocked" if _blocked else "no_challenges"),
        "validation_status": str(validation_status or ""),
        # An unchallenged reading and an unchallengeable run are different records.
        "reading_was_challenged": bool(challenges) or not _blocked,
    }
    for idx, item in enumerate(challenges, 1):
        dossier["challenges"].append({
            ...
            "anchor_kind": str(item.get("anchor_kind") or "factual_pointer"),
            "factual_pointer": dict(item.get("factual_pointer") or {}),
            "absence_pointer": dict(item.get("absence_pointer") or {}),
            "reviewer_pointer": dict(item.get("reviewer_pointer") or {}),
            "requirement_pointer": dict(item.get("requirement_pointer") or {}),
            "claim_id": str(item.get("claim_id") or (item.get("claim_test") or {}).get("claim_id") or ""),
            "claim_test": dict(item.get("claim_test") or {}),
            "evidence_id": str((item.get("factual_pointer") or {}).get("evidence_id") or ""),
