...
**Scope of the trace.** The tracer covers the twenty modules named in `tools/trace_unreached_guards.py` (kit v20 added
the scheduler and the inbox; kit v21 the regulatory watch intel service; kit v22 the twin generator and adjudication
log, the arena, its cases, contestants and judge, the requirement basis and the state-path module). The
...
Summary: 0 registered; all 12 previously registered guards are reached by exact-message tests (kit v19). Trace scope: twenty modules; command-line tools not traced.
