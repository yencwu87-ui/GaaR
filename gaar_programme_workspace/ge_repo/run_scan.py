...
xlsx = Path(a.playbook) if a.playbook else next(workbench_data().glob("*.xlsx"))
...
workbench_data().mkdir(exist_ok=True)
...
(workbench_data() / f"proposals_{stamp}.json").write_text(json.dumps(
...
(workbench_data() / f"gap_analysis_{stamp}.md").write_text("\n".join(md))
print(f"\nWrote {workbench_data()}/proposals_{stamp}.json and gap_analysis_{stamp}.md. Nothing was written to the playbook.")
