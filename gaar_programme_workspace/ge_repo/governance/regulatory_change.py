...
from .triangulation import SourceRegistry, build_change_alerts, digest
...
def build_draft(proposed_controls: dict, *, source_title: str, source_reference: str,
                ...
                notes: str = "", source_id: str | None = None, as_of: str | None = None) -> dict:
    ...
    if source_id:
        ...
        src = registry.sources.get(source_id)
        if src:
            source_snapshot = {"source_id": source_id, "status": src.lifecycle_status, "normative_status": src.normative_status, "publication_date": src.publication_date, "effective_from": src.effective_from, "effective_to": src.effective_to, "content_sha256": src.content_sha256}
            source_alerts = build_change_alerts(registry, as_of=as_of or date.today().isoformat())
    proposal["regulatory_change"] = {
        ...
        "source_change_alerts": source_alerts,
        "source_snapshot_digest": digest(source_snapshot) if source_snapshot else None,
    }
