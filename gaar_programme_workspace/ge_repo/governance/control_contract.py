...
ALIASES = {
    ...
    'MGF':'MGF Agentic', 'MGF_Agentic':'MGF Agentic', 'MGF Agentic':'MGF Agentic', 'SAFR':'SAFR'
}
...
def requirement_context(control_id: str, framework: str = '') -> dict:
    ...
    # WB-062: provenance travels to the point of use. `requirement_authority` and
    # `test_provenance` were computed by the exporter, stored in the contract, and dropped here —
    # so the assessor and the challenger judged against `draft` elements with nothing anywhere
    # saying they were drafts. For 165 of 195 controls those elements are a restatement of the
    # control title, and that fact has to reach the prompt and the record.
    return {'source':c.get('requirement_source'),'control_id':control_id,'requirement':c.get('requirement',''),
            'elements':c.get('elements') or [],'boundary':c.get('boundary') or {},
            'authority':c.get('requirement_authority'),
            'test_provenance':c.get('test_provenance',''),
            'is_draft':str(c.get('requirement_authority') or '').strip().lower() != 'authoritative_internal_interpretation'}
