...
def test_unverifiable_quote_is_rejected(monkeypatch):
    """The guard still fires; the contract around it changed from raise to blocked envelope.

    WB-102: this test asserted `pytest.raises`. The pass now exhausts its repair retries and
    returns a record carrying `validation_status: "blocked"` and the guard's message, which is
    the better behaviour — a caller gets an auditable artefact rather than an exception. What
    matters is that the unverifiable quote never reaches `challenges`, and that the envelope
    says the run was blocked rather than presenting an empty list as a clean result.
    """
    _stub(monkeypatch, {"overall_reasoning": "x",
    ...
    assert "not verifiable" in out["validation_error"]


def test_a_blocked_run_is_distinguishable_from_a_silent_one(monkeypatch):
    """`challenges: []` alone cannot tell the two apart — only the envelope can."""
    _stub(monkeypatch, {"overall_reasoning": "x",
                        "challenges": [_challenge(element_id="e3", text="Post-implementation verification and closure or exception handling are required.")],
                        "sharpest": "s", "unaddressed": []})
    silent = CH.challenge_disagreement(_Ctl(), EVIDENCE, BLIND, AI, DIFF)
    _stub(monkeypatch, {"overall_reasoning": "x",
                        "challenges": [_challenge(quote="the CAB minutes record a verbal approval")],
                        "sharpest": "s", "unaddressed": []})
    blocked = CH.challenge_disagreement(_Ctl(), EVIDENCE, BLIND, AI, DIFF)
    assert silent["challenges"] == blocked["challenges"] == []
    assert silent.get("validation_status") != "blocked"
