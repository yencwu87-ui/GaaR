...
PARAMS = {
    ...
    # `fields_truthy` reads p["source"] and p["fields"], with an optional p["where"]. It had no
    # entry here, so the sweep below silently skipped it — which is exactly the failure the test
    # above exists to catch, and it had been sitting caught.
    "fields_truthy":     {"source": "s", "fields": ["a"]},
}
