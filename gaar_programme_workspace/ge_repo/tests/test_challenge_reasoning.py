...
def test_reviewer_claim_vetting_function_exists_and_is_conservative(monkeypatch):
    import challenge as ch
    monkeypatch.setattr(ch, "_challenge_llm", lambda *a, **k: '{"claims":[{"claim_id":"C1","claim":"The evidence proves monitoring operated monthly","element_id":"e1","reviewer_position":"met","evidence_assessment":"unsupported","evidence_reason":"The evidence mentions logging but not monthly review.","evidence_quotes":[],"confidence":"high"}]}')
    control = {"id": "M3.12", "title": "Test", "lib": "", "req": "Test requirement"}
    label = {"sufficiency":"full","maturity":5,"reason":"The evidence proves monitoring operated monthly","element_verdicts":[{"element_id":"e1","status":"met"}]}
    claims, err = ch._run_claim_vet(control, "--- Source: WB-025 ---\nLogging is enabled.", label)
    assert err is None
    assert claims[0]["evidence_assessment"] == "unsupported"
