...
def test_retriever_has_bm25_fallback():
    """The fallback itself, not whatever the rank_bm25 import resolved to. With the dependency
    installed this previously tested rank_bm25, whose IDF is exactly log(1.0) = 0 for a term in
    one of two documents — so the assertion failed on every machine that had it."""
    from retriever import FallbackBM25 as BM25Okapi
