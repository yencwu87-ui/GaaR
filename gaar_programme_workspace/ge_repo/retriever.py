...
class FallbackBM25:
    ...
    Defined unconditionally so it can be imported and tested on its own. Previously it
    existed only when rank_bm25 was absent, so the test named after it silently exercised
    rank_bm25 on any machine that had the dependency — and failed there, because the two
    use different IDF formulas. This one smooths with a +1 and is always positive;
    rank_bm25's is log((N-df+0.5)/(df+0.5)), exactly zero for a term in half the corpus.
    """
